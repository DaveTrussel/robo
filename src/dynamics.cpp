...
namespace robo{
    ...
    int Dynamics::calculate_torques(const VectorXd& q_, const VectorXd& dq_, const VectorXd& ddq_,
        ...
        for(int iter_link=0; iter_link<chain.nr_links; ++iter_link){
            ...
            // TODO find proper naming for transformations
            trans_to_parent[iter_link] = link.pose(q); // trans_from_parent from parent to child, gives the frame of the tip of current link expressed in current link root coordinates
            trans_from_parent = trans_to_parent[iter_link].inverse(); // trans_from_parent from child to parent expressed in the the current link root frame 
            ...
            if(iter_link==0){
                ...
                accelerations[iter_link] = trans_from_parent * gravity +
                                           motion_subspace[iter_link] * ddq +
                                           velocities[iter_link].cross(joint_twist);
            }
            else{
                velocities[iter_link]    = trans_from_parent * velocities[iter_link-1] + joint_twist;
                accelerations[iter_link] = trans_from_parent * accelerations[iter_link-1] +
                                           motion_subspace[iter_link] * ddq +
                                           velocities[iter_link].cross(joint_twist);
            }

            // calculate wrench acting on current link (this could also be part of the backward recursion)
        ...
        for(int iter_link=chain.nr_links-1; iter_link>=0; --iter_link){
            ...
            if(iter_link>0){
                // add wrench of this link to lower link's wrench (trans_from_parent into lower frame)
                wrenches[iter_link-1] += trans_to_parent[iter_link]*wrenches[iter_link];;
